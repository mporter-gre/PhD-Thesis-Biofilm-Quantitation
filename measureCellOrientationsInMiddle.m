function props = measureCellOrientationsInMiddle(session, imageId)
% props = measureCellOrientationsInMiddle(session, imageId)
...
numROIs = length(rois);
for thisROI = 2:numROIs
    roi = rois{thisROI};
    patch = getPatchFromRectROI(session, imageId, roi, 0);
    patchSeg = logical(seg2D(patch,0,0,20));
    
    props{thisROI-1} = regionprops(patchSeg, 'Orientation', 'MajorAxisLength', 'Eccentricity');
end
disp('check');
