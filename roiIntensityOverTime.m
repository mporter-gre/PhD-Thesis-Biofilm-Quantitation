function [meanPatch, minMaxMeanPatch, FXMinMax, fdhm] = roiIntensityOverTime(session, imageId, c)
% [meanPatch, minMaxMeanPatch, FXMinMax, fdhm] = roiIntensityOverTime(session, imageId, c)
...
for thisROI = 1:numROIs
    ...
    FXN = gradient(minMaxMeanPatch(thisROI,:));
    [f, gof] = fit(1:length(FXN)', FXN', 'gauss1');
    %FXMinMax(thisROI) = max(FXN, [], 2);
